...
title('randn: x = N(0,1)'); grid on; xlabel('sample'); ylabel('x'); hold on;
plot(xval, e, '-r', 'linewidth', 2);
plot(xval, s, '-', 'color', c.darkred, 'linewidth', 2);
legend('randn realisations', 'sample mean', 'sample standard deviation');
...
hold on; grid on; title('Sample Mean Estimation');
theoretical_mean(1:n_realisations) = 0;
plot(xval, theoretical_mean, '-r', 'LineWidth', 1.5);
plot(0, 0.5, 'ow');
plot(0, -0.5, 'ow');
legend('sample mean', 'theoretical mean'); xlabel('realisation'); ylabel('mean')
...
plot(0, 0, 'ow');
plot(0, 1.5, 'ow');
legend('sample st deviation','theoretical st deviation', 'location','southwest'); xlabel('realisation'); ylabel('standard deviation')
...
grid on; title('10 bins; 1000 samples'); xlabel('x'); ylabel('f_X(x)');
legend('PSD Estimation','Theoretical PSD','location','southwest');
...
bar(h_centers, h_counts, 'FaceColor', c.orange, 'EdgeColor', c.orange);
hold on; plot(xval2, theoretical_psd, '-r', 'LineWidth', 2);
grid on; title('100 bins; 1000 samples'); xlabel('x'); ylabel('f_X(x)');
clear n n_bins x1 h_counts h_centers
...
bar(h_centers, h_counts, 'FaceColor', c.orange, 'EdgeColor', c.orange);
hold on; plot(xval2, theoretical_psd, '-r', 'LineWidth', 2);
grid on; title('10 bins; 100 000 samples'); xlabel('x'); ylabel('f_X(x)');
clear n n_bins x1 h_counts h_centers
...
bar(h_centers, h_counts, 'FaceColor', c.orange, 'EdgeColor', c.orange);
hold on; plot(xval2, theoretical_psd, '-r', 'LineWidth', 2);
grid on; title('100 bins; 100 000 samples'); xlabel('x'); ylabel('f_X(x)');
clear n n_bins x1 h_counts h_centers